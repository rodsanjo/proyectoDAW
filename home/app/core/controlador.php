...
class Controlador extends \core\Clase_Base {	
	...
	public function js_script_tag($path_y_fichero, array &$datos = null) {
		...
		if ( is_file($path_js_fichero)) {
			...
			$path_js_fichero1 = $path_js_fichero;
			$path_js_fichero = PATH_HOME.$path_y_fichero;
			if ( is_file($path_js_fichero)) {
				$url_js_fichero = \core\URL::root_esmvcphp()."$path_y_fichero";
			}
			else {
				throw new \Exception(__METHOD__." => El fichero js $path_y_fichero no existe ni en $path_js_fichero1 ni en $path_js_fichero.");
			}
		}
	...
	public function js_script_vistas_tag($nombre_fichero, array &$datos = null) {
		...
		if ( ! preg_match('/\.js$/i', $nombre_fichero))
			$nombre_fichero .= '.js';
		$path_y_fichero = "recursos/js/".\core\Distribuidor::get_controlador_instanciado()."/$nombre_fichero";
	...
	public function css_link_tag($path_y_fichero, array &$datos = null) {
		...
		$path_css_fichero = PATH_ROOT.$path_y_fichero;
		
		if ( is_file($path_css_fichero)) {
			$url_css_fichero = \core\URL::http('')."$path_y_fichero";
		}
		else {
			$path_css_fichero1 = $path_css_fichero;
			$path_css_fichero = PATH_HOME.$path_y_fichero;
			if ( is_file($path_css_fichero)) {
				$url_css_fichero = \core\URL::root_esmvcphp()."$path_y_fichero";
			}
			else {
				throw new \Exception(__METHOD__." => El fichero css $path_y_fichero no existe ni en $path_css_fichero1 ni en $path_css_fichero.");
			}
		}
	...
	public function css_link_vistas_tag($nombre_fichero, array &$datos = null) {
		...
		$path_y_fichero = "recursos/css/".\core\Distribuidor::get_controlador_instanciado()."/$nombre_fichero";
