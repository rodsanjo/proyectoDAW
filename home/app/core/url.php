...
class URL {
	...
	public static function root_home() {
	
		$url = (isset($_SERVER['REQUEST_SCHEME']) ? $_SERVER['REQUEST_SCHEME'] : ($_SERVER['SERVER_PORT']==80 ? "http" : "https"))."://".$_SERVER['SERVER_NAME'].str_replace(APPLICATION_FOLDER."/index.php", "esmvcphp/", $_SERVER['SCRIPT_NAME']); 
...
//				// Patrón url amigable
	...
	private static function query_string(array $query_string = null, $withLang = true, $administrator = true) {
		...
		foreach ($query_string as $key => $value) {
			$url .= (strlen($url) == 1 ? "" : "&")."p".($key+1)."=$value";  //Si es el último elemento no se pone &
	...
	public static function actual() {
            if(\core\Configuracion::$session_activate)
	...
	public static function url_btn_volver() {
        ...
        /**
         * Genera la ruta absoluta dandole una ruta relativa
         * 
         * Autor: jergo
         * 
         * @param type $rura_rel
         * @return type
         */
        public static function ruta_absoluta($rura_rel='') {			
		return URL_ROOT.$rura_rel;
	}
}